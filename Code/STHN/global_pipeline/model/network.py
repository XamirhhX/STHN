...
def get_pretrained_model(args):
    ...
    if not os.path.exists(file_path):
        if gdd is None:
            raise ImportError("google_drive_downloader is required to download pretrained backbones")
...
def get_backbone(args):
    ...
    if args.backbone.startswith("resnet"):
        ...
        if cct_14_7x2_384 is None:
            raise ImportError("timm is required for CCT backbones")
        ...
        if ViTModel is None:
            raise ImportError("transformers is required for ViT backbones")
