...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="global",
        choices=["global", "spiral"],
        help="Use the original global nearest-neighbor search or the prior-centered spiral search.",
    )
    parser.add_argument(
        "--spiral_grid_step_m",
        type=float,
        default=None,
        help="Tile/grid spacing for spiral ordering. If omitted, it is inferred from database coordinates.",
    )
    parser.add_argument(
        "--spiral_search_radius_m",
        type=float,
        default=-1.0,
        help="Maximum distance from the prior to evaluate. If <= 0, all database tiles are eligible.",
    )
    parser.add_argument(
        "--spiral_stop_distance",
        type=float,
        default=None,
        help="Optional squared descriptor-distance threshold for early stopping.",
    )
    parser.add_argument(
        "--spiral_prior_noise_m",
        type=float,
        default=0.0,
        help="Gaussian noise added to query coordinates to simulate imperfect prior estimates during evaluation.",
    )
    parser.add_argument(
        "--spiral_handoff_top_n",
        type=int,
        default=3,
        help="Number of retrieval candidates expanded for the later homography stage.",
    )
    parser.add_argument(
        "--spiral_handoff_neighbors",
        type=int,
        default=4,
        help="Number of nearest shifted-grid overlap tiles added per Top-N candidate.",
    )
    parser.add_argument(
        "--spiral_report_name",
        type=str,
        default="spiral_retrieval_report",
        help="Base filename for spiral retrieval JSON/CSV outputs.",
    )
    parser.add_argument(
        "--save_retrieval_embeddings",
        type=str,
        default=None,
        help="Optional .npz path to store precomputed database/query embeddings and coordinates.",
    )
    parser.add_argument(
        "--database_image_source",
        type=str,
        default="map",
        choices=["map", "h5"],
        help="Read database images by cropping the map PNG or directly from *_database.h5 image_data.",
    )
    parser.add_argument(
        "--database_crop_size_px",
        type=int,
        default=None,
        help="Crop size in pixels when reading database images from the map. Defaults to --resize width.",
    )
    parser.add_argument(
        "--use_best_n",
        type=int,
    ...
    if args.search_strategy == "spiral" and args.efficient_ram_testing:
        raise ValueError("Spiral search requires in-memory embeddings; disable --efficient_ram_testing.")

    if args.spiral_grid_step_m is not None and args.spiral_grid_step_m <= 0:
        raise ValueError("--spiral_grid_step_m must be positive when provided")

    if args.spiral_stop_distance is not None and args.spiral_stop_distance <= 0:
        raise ValueError("--spiral_stop_distance must be positive when provided")

    if args.spiral_prior_noise_m < 0:
        raise ValueError("--spiral_prior_noise_m must be non-negative")

    if args.spiral_handoff_top_n < 0:
        raise ValueError("--spiral_handoff_top_n must be non-negative")

    if args.spiral_handoff_neighbors < 0:
        raise ValueError("--spiral_handoff_neighbors must be non-negative")
