...
import datasets_ws
import h5py
from spiral_retrieval import (
    build_homography_candidate_sets,
    compute_ranking_metrics,
    make_prior_utms,
    save_spiral_outputs,
    search_dataset_spiral,
)
...
def test(args, eval_ds, model, test_method="hard_resize", pca=None, visualize=False):
    ...
    if args.search_strategy == "spiral" and test_method in ["nearest_crop", "maj_voting"]:
        raise NotImplementedError(
            "Spiral search supports one descriptor per query. Use hard_resize, "
            "single_query, central_crop, or five_crops."
        )
    ...
    if args.save_retrieval_embeddings:
        embedding_dir = os.path.dirname(args.save_retrieval_embeddings)
        if embedding_dir:
            os.makedirs(embedding_dir, exist_ok=True)
        np.savez_compressed(
            args.save_retrieval_embeddings,
            database_features=database_features,
            queries_features=queries_features,
            database_utms=eval_ds.database_utms,
            queries_utms=eval_ds.queries_utms,
            database_paths=np.asarray(eval_ds.database_paths),
            queries_paths=np.asarray(eval_ds.queries_paths),
        )
        logging.info(f"Saved retrieval embeddings to {args.save_retrieval_embeddings}")

    logging.debug("Calculating recalls")
    if args.search_strategy == "spiral":
        search_radius_m = (
            args.spiral_search_radius_m
            if args.spiral_search_radius_m is not None and args.spiral_search_radius_m > 0
            else None
        )
        top_k = max(max(args.recall_values), args.spiral_handoff_top_n)
        prior_utms = make_prior_utms(
            eval_ds.queries_utms,
            prior_noise_m=args.spiral_prior_noise_m,
            seed=args.seed,
        )
        distances, predictions, spiral_stats = search_dataset_spiral(
            queries_features=queries_features,
            database_features=database_features,
            database_utms=eval_ds.database_utms,
            prior_utms=prior_utms,
            top_k=top_k,
            grid_step_m=args.spiral_grid_step_m,
            search_radius_m=search_radius_m,
            stop_distance=args.spiral_stop_distance,
        )
        spiral_metrics = compute_ranking_metrics(
            predictions=predictions,
            positives_per_query=eval_ds.get_positives(),
        ...
        homography_candidates = build_homography_candidate_sets(
            ...
            top_n=args.spiral_handoff_top_n,
            overlap_neighbors=args.spiral_handoff_neighbors,
        )
        spiral_output_paths = save_spiral_outputs(
            save_dir=args.save_dir,
            report_name=args.spiral_report_name,
            config={
                ...
                "prior_noise_m": args.spiral_prior_noise_m,
                "top_k": top_k,
                "handoff_top_n": args.spiral_handoff_top_n,
                "handoff_neighbors": args.spiral_handoff_neighbors,
            },
            metrics=spiral_metrics,
            predictions=predictions,
            distances=distances,
            positives_per_query=eval_ds.get_positives(),
            query_paths=eval_ds.queries_paths,
            database_paths=eval_ds.database_paths,
            queries_utms=eval_ds.queries_utms,
            database_utms=eval_ds.database_utms,
            stats=spiral_stats,
        ...
        logging.info(
            "Spiral search summary: "
            f"accuracy@1={spiral_metrics['accuracy_at'].get('1', 0.0):.1f}, "
            f"accuracy@3={spiral_metrics['accuracy_at'].get('3', 0.0):.1f}, "
            f"accuracy@5={spiral_metrics['accuracy_at'].get('5', 0.0):.1f}, "
            f"mean_runtime_ms={spiral_metrics['runtime']['mean_ms']:.3f}, "
            f"mean_tiles={spiral_metrics['tiles_evaluated']['mean']:.1f}"
        )
        logging.info(f"Spiral report saved to {spiral_output_paths['json']}")
        del database_features
    elif args.prior_location_threshold == -1:
