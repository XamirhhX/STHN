...
class BaseDataset(data.Dataset):
    ...
    def __init__(
        ...
        self.database_image_source = getattr(args, "database_image_source", "map")
        ...
        if self.database_image_source == "h5" and "image_data" not in database_folder_h5_df:
            raise ValueError(
                f"{self.database_folder_h5_path} does not contain image_data; "
                "--database_image_source h5 requires database images in H5."
            )
        if self.database_image_source == "map":
            self.database_folder_map_df = F.to_tensor(Image.open(self.database_folder_map_path))
        else:
            self.database_folder_map_df = None
    ...
    def __getitem__(self, index):
        ...
        if self.is_index_in_queries(index):
            ...
            if self.database_image_source == "h5":
                img = self._find_img_in_h5(index, "database")
            else:
    ...
    def _find_img_in_map(self, index, database_query_split=None, extended=False):
        ...
        crop_size_px = getattr(self.args, "database_crop_size_px", None) or self.args.resize[0]
        area = (int(center_cood[1]) - crop_size_px//2, int(center_cood[0]) - crop_size_px//2,
                int(center_cood[1]) + crop_size_px//2, int(center_cood[0]) + crop_size_px//2)
